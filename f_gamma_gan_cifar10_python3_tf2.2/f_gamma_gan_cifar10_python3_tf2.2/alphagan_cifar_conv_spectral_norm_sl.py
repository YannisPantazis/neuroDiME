...
sess_name = sys.argv[7] # name of the session run
...
def ConvBlock_spec_norm(name, input_dim, output_dim, filter_size, inputs, resample=None, no_dropout=False, labels=None):
    """
    resample: None, 'down', or 'up'
    """
    if resample=='down':
...
def OptimizedConvBlockDisc1_spec_norm(inputs):
...
def Discriminator(inputs, labels):
    ...
    output = OptimizedConvBlockDisc1_spec_norm(output)
    output = ConvBlock_spec_norm('Discriminator.2', DIM_D, DIM_D, 3, output, resample='down')
    output = ConvBlock_spec_norm('Discriminator.3', DIM_D, DIM_D, 3, output, resample=None)
    output = ConvBlock_spec_norm('Discriminator.4', DIM_D, DIM_D, 3, output, resample=None)
...
with tf.Session() as session:
    ...
    resume_iter = load_checkpoint('cifar_resnet_sn/'+str(sess_name)+'/checkpoints/')
    ...
    for iteration in range(resume_iter+1, ITERS):
       ...
       # lib.plot.plot('samples/cifar10_resnet/cost', _disc_cost)
        ...
        if iteration % INCEPTION_FREQUENCY == INCEPTION_FREQUENCY-1:
            ...
            save_checkpoint('cifar_resnet_sn/'+str(sess_name)+'/checkpoints/', iteration)
