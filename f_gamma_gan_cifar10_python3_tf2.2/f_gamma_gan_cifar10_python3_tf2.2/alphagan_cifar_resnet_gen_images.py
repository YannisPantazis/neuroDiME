...
LAMBDA =float(sys.argv[6]) # Gradient penalty lambda hyperparameter: original value =10
gp_1_sided=False #gradient penalty: 1 sided or 2 sides
if  int(sys.argv[7])==1:
    gp_1_sided=True
sess_name = sys.argv[8] # name of the session run
...
with tf.Session() as session:
    ...
    iteration = load_checkpoint('cifar_resnet/'+str(sess_name)+'/checkpoints/')
    ...
    all_samples = all_samples.reshape((-1, 3, 32, 32)).transpose(0,2,3,1)
    gen_image_file = 'cifar_resnet/'+str(sess_name)+'/gen_images_50k_alpha_' + str(alpha) + '_rev_' + str(rev)+ '_set'+str(j+itr) +'_iteration'+ str(iteration)+ '.npy'
