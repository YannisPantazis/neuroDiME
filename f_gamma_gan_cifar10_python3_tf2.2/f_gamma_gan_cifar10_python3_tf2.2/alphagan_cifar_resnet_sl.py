...
N_CRITIC =int(sys.argv[2]) # How many discriminator iterations to train per generator
alpha = float(sys.argv[3])  # cumgan parameters
rev = int(sys.argv[4])  # cumgan parameters
itr=int(sys.argv[5])    # total number of runs
LR=float(sys.argv[6])   # Initial learning rate: original value=2e-4
LAMBDA =float(sys.argv[7]) # Gradient penalty lambda hyperparameter: original value =10
gp_1_sided=False #gradient penalty: 1 sided or 2 sides
if  int(sys.argv[8])==1:
    gp_1_sided=True
sess_name = sys.argv[9] # name of the session run
...
G_gp = True # Gradient penalty added in Generator
j=0
...
if not os.path.exists('cifar_resnet/'+str(sess_name)+'/'):
    os.makedirs('cifar_resnet/'+str(sess_name)+'/')
...
with tf.Session() as session:
    ...
    inception_file = 'cifar_resnet/'+str(sess_name)+'/inception_scores_alpha_' + str(alpha) + '_rev_' + str(rev)+ '_set'+str(j+itr) + '.csv'
    ...
    for i, device in enumerate(DEVICES_B):
        with tf.device(device):
            ...
            interpolates = real_data + (eta*differences)
            gradients = tf.gradients(Discriminator(interpolates, labels)[0], [interpolates])[0]
            slopes_2=tf.reduce_sum(tf.square(gradients), reduction_indices=[1])
            slopes = tf.sqrt(slopes_2)
                
            if gp_1_sided:
                gradient_penalty = tf.reduce_mean(tf.math.maximum(0.,(slopes_2-1.)))
                print('1_sided GP')
            else:
                gradient_penalty = tf.reduce_mean((slopes-1.)**2)
                print('2_sided GP')

            gradient_penalty = LAMBDA*gradient_penalty
    ...
    resume_iter = load_checkpoint('cifar_resnet/'+str(sess_name)+'/checkpoints/')
    ...
    for iteration in range(resume_iter+1, ITERS):
       ...
       # lib.plot.plot('samples/cifar10_resnet/cost', _disc_cost)
        if CONDITIONAL and ACGAN:
            lib.plot.plot('samples/cifar10_resnet/wgan', _disc_wgan)
            lib.plot.plot('samples/cifar10_resnet/acgan', _disc_acgan)
            lib.plot.plot('samples/cifar10_resnet/acc_real', _disc_acgan_acc)
            lib.plot.plot('samples/cifar10_resnet/acc_fake', _disc_acgan_fake_acc)
        lib.plot.plot('samples/cifar10_resnet/time', time.time() - start_time)

        if iteration % INCEPTION_FREQUENCY == INCEPTION_FREQUENCY-1:
            inception_score = get_inception_score(10000)
            print("epoch: ", iteration," Inception scores: ",inception_score[0]) 
            inception_scores.append(inception_score[0])

            save_checkpoint('cifar_resnet/'+str(sess_name)+'/checkpoints/', iteration)
           ...
           # lib.plot.plot('samples/cifar10_resnet/inception_50k', inception_score[0])
           # lib.plot.plot('samples/cifar10_resnet/inception_50k_std', inception_score[1])
        ...
        if iteration % 100 == 99:
            ...
            #lib.plot.plot('samples/cifar10_resnet/dev_cost', np.mean(dev_disc_costs))
