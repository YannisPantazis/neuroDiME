...
alpha = float(sys.argv[2])  # f div param
...
N_CRITIC = int(sys.argv[6]) # How many discriminator iterations to train per generator
beta_1 = float(sys.argv[7])  #Adam Optimizer momentum parameters
beta_2 = float(sys.argv[8])
...
with tf.Session() as session:
           ...
           #losses
            if alpha==0:    # gradient penalty Standard WGAN loss
                ...
                if rev == 1:
                    ...
                    disc_cost_tot=disc_cost_real-disc_cost_fake
            elif alpha==-2: #this is the code for JS
                if rev == 0:
                    disc_cost_fake =tf.reduce_mean(-tf.nn.relu(1.0+disc_fake)) #tf.reduce_mean(tf.math.log(1.0-disc_fake))
                    disc_cost_real = tf.reduce_mean(-tf.nn.relu(1.0-disc_real)) #tf.reduce_mean(tf.math.log(disc_real))
                    disc_cost_tot=disc_cost_real+disc_cost_fake        
                    print(i)
                    print("disc-JS")
        ...
        with tf.device(device):
            ...
            if CONDITIONAL and ACGAN:
                ...
                if alpha==0:    # gradient penalty Standard WGAN loss
                    ...
                    if rev == 0:
                        gen_c=-tf.reduce_mean(disc_fake) #-tf.reduce_mean(tf.math.log(disc_fake))
                        print('gen-JS')
                else: #reverse generalized alpha GAN
                    if rev == 1:
                        gen_c=tf.reduce_mean(disc_fake)
    ...
    gen_opt = tf.train.AdamOptimizer(learning_rate=LR*decay, beta1=beta_1, beta2=beta_2)
    disc_opt = tf.train.AdamOptimizer(learning_rate=LR*decay, beta1=beta_1, beta2=beta_2)
